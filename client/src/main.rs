...
fn main() -> Result<()> {
    ...
    match opts.command {
        RaydiumCpCommands::CreateAmmConfig {
            ...
            let create_amm_config_ix = create_amm_config(
                &pool_config,
                payer.pubkey(),
                amm_config,
                index,
        ...
        RaydiumCpCommands::InitializePool {
            ...
            let mint0 = Pubkey::from_str(&mint0).unwrap();
            let mint1 = Pubkey::from_str(&mint1).unwrap();
            ...
            println!("token0program {}", token_0_program);
            println!("token1 {}", token_1_program);
            ...
            let initialize_pool_instr = initialize_pool_instr(
                ...
                solar_cp_swap::create_pool_fee_reveiver::id(),
                init_amount_0,
                init_amount_1,
                open_time,
            )?;
        ...
        RaydiumCpCommands::Deposit {
            ...
            let pool_state: solar_cp_swap::states::PoolState = program.account(pool_id)?;
            // load account
            ...
            // calculate amount
            let results = solar_cp_swap::curve::CurveCalculator::lp_tokens_to_trading_tokens(
                u128::from(lp_token_amount),
                u128::from(pool_state.lp_supply),
                u128::from(total_token_0_amount),
                u128::from(total_token_1_amount),
                solar_cp_swap::curve::RoundDirection::Ceiling,
            )
            .ok_or(solar_cp_swap::error::ErrorCode::ZeroTradingTokens)
            .unwrap();
            println!(
        ...
        RaydiumCpCommands::SwapBaseIn {
            ...
            let amm_config_state = deserialize_anchor_account::<solar_cp_swap::states::AmmConfig>(
            ...
            let (
                ...
                (
                    solar_cp_swap::curve::TradeDirection::ZeroForOne,
                ...
                (
                    solar_cp_swap::curve::TradeDirection::OneForZero,
                    total_token_1_amount,
                    total_token_0_amount,
                    user_input_token,
                    spl_associated_token_account::get_associated_token_address(
            ...
            let result = solar_cp_swap::curve::CurveCalculator::swap_base_input(
        ...
        RaydiumCpCommands::SwapBaseOut {
            ...
            let result = solar_cp_swap::curve::CurveCalculator::swap_base_output(
